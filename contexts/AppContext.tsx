...
export const [AppProvider, useApp] = createContextHook(() => {
  ...
  const setAuth = useCallback(async (token: string, user: User) => {
    ...
    setState((prev) => ({ ...prev, authToken: token, user }));
  }, []);
  ...
  useEffect(() => {
    if (state.instanceUrl) {
      ...
      client.interceptors.request.use(
        (config) => {
          if (state.authToken) {
            config.headers['Cookie'] = `remember_token=${state.authToken}`;
          }
          return config;
        },
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async (data) => {
      if (data.success && !data.error && data.data && data.data.user) {
        const token = data.data.user.remember_token;
        if (token) {
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      if (!state.instanceUrl || !state.authToken) return;

      await axios.get(`${state.instanceUrl.replace(/\/$/, '')}/api/user/auth/logout`, {
        headers: {
          Cookie: `remember_token=${state.authToken}`,
        },
      });
    },
    onSuccess: async () => {
